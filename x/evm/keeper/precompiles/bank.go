...
func (bc *BankContract) Run(evm *vm.EVM, input []byte, caller common.Address, value *big.Int, readonly bool) ([]byte, error) {
	...
	case string(MintMethod.ID):
		...
		err = stateDB.ExecuteNativeAction(func(ctx sdk.Context) error {
			...
			originAmount := bc.bankKeeper.GetBalance(bc.ctx, addr, denom).Amount.BigInt()
			dirtyAmount := new(big.Int).Add(originAmount, amount)
			changed := new(big.Int).Sub(dirtyAmount, originAmount)
			switch changed.Sign() {
			case 1:
				amt := sdk.NewCoins(sdk.NewCoin(denom, sdkmath.NewIntFromBigInt(changed)))
			...
			case -1:
				...
				if err := bc.bankKeeper.SendCoinsFromAccountToModule(ctx, addr, types.ModuleName, amt); err != nil {
