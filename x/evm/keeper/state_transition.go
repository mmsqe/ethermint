...
func (k *Keeper) NewEVM(
	...
	return k.evmConstructor(blockCtx, txCtx, k, cfg.ChainConfig, vmConfig, k.customPrecompiles)
}
...
func (k *Keeper) ApplyTransaction(tx *ethtypes.Transaction) (*types.MsgEthereumTxResponse, error) {
	...
	if len(receipt.Logs) > 0 {
		...
		k.SetBlockBloomTransient(receipt.Bloom.Big())
	}
	...
	k.ResetGasMeterAndConsumeGas(totalGasUsed)
	return res, nil
}
...
func (k *Keeper) ApplyMessageWithConfig(
	...
	if rules := cfg.ChainConfig.Rules(big.NewInt(k.Ctx().BlockHeight()), cfg.ChainConfig.MergeNetsplitBlock != nil); rules.IsBerlin {
		k.PrepareAccessList(msg.From(), msg.To(), evm.ActivePrecompiles(rules), msg.AccessList())
	}
	...
	leftoverGas += k.GasToRefund(temporaryGasUsed, refundQuotient)
