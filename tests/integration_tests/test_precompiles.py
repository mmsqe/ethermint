...
@pytest.mark.parametrize("suffix", ["", "Delegate"])
def test_precompiles(ethermint, suffix):
    ...
    data = {"from": addr}
    tx = contract.functions["nativeMint" + suffix](amount).buildTransaction(data)
    ...
    tx = contract.functions["nativeMintRevert" + suffix](amount).buildTransaction(
