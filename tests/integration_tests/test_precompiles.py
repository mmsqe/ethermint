...
def test_nested(ethermint):
    ...
    amount = 100
    res = deploy_contract(w3, CONTRACTS["TestBank"])
    contract = deploy_contract(w3, CONTRACTS["TestBankCaller"])
    data = {"from": addr}
    tx = contract.functions.mint(res.address, amount).build_transaction(data)
